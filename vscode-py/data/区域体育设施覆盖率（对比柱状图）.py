...
# 设置字体大小
label_font_size = 12  # 数值标签的字体大小
axis_font_size = 14   # 坐标轴标签的字体大小
...
for bar, cov, area in zip(bars, coverage, per_capita):
    ...
    plt.text(bar.get_x() + bar.get_width()/2, height + label_offset_top,
             f"{cov}%", ha='center', va='bottom', fontsize=label_font_size)
    ...
    plt.text(bar.get_x() + bar.get_width()/2, height + label_offset_bottom,
             f"{area}m²", ha='center', va='top', color='white', fontsize=label_font_size)

plt.title("东中西部体育设施覆盖率对比 (2023)", pad=20, fontsize=axis_font_size)
plt.ylabel("覆盖率 (%)", fontsize=axis_font_size)
plt.xlabel("地区", fontsize=axis_font_size)  # 添加x轴标签并设置字体大小
plt.ylim(0, 70)
